...
public class SessionManager {
	...
	public void cleanup() {
		...
		// I can't just for-each this in the stream as it will throw a ConcurrentModificationException
		List<GamePlayer> dead = sessions.values()
										.stream()
	...
	public List<GamePlayer> getPlayers(InetAddress address) {
		return connections.containsKey(address) ? connections.get(address).stream().map(sessions::get).collect(Collectors.toList()) : Collections.emptyList();
	}
