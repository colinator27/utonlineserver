		...
		executor.execute(() -> {
			...
			while(running.get()) {
					...
					if(!ratelimited.getAndSet(true)) {
	            		...
	            		for(GamePlayer p : sessionManager.getPlayers()) {
	            			if(p.connAddress.equals(owner)) {
	            				server.sendPacket(p.connAddress, p.connPort, new PacketBuilder(OutboundPacketType.RATELIMIT_WARNING, send));
	            			}
	            		}
					}
					continue;
				...
				try {
					...
					case LOGIN: {
						if(server.properties.disallowSameIP && !sessionManager.getPlayers(owner).isEmpty()) {
							LOG.logger.info("Rejected session request from " + owner + ":" + packet.getPort() + " (same IPs disallowed)");
							continue;
						}
						player = sessionManager.createPlayer(owner, packet.getPort());
						if(player == null) {
							LOG.logger.info("Rejected session request from " + owner + ":" + packet.getPort() + " (server is full)");
							server.sendPacket(
									owner, 
									packet.getPort(), 
									new PacketBuilder(OutboundPacketType.KICK_MESSAGE, send)
										.addString("Cannot join this server; it is at a maximum capacity of " + server.properties.maxPlayers + " players.")
							);
							continue;
						}
						LOG.logger.info(String.format(
								"Created session for %s:%s (id = %d, uuid = %s)",
								owner,
								packet.getPort(),
								player.id,
								player.uuid
					...
					case HEARTBEAT: {
						...
						if(player != null) {
							...
							server.sendPacket(owner, packet.getPort(), new PacketBuilder(OutboundPacketType.HEARTBEAT, send));
						}
					} break;
					case PLAYER_CHANGE_ROOM: {
						...
						if(player != null) {
							...
							player.lastPacketTime = System.currentTimeMillis();
							if(server.validatePlayerVisuals(player, spriteIndex, imageIndex, x, y)) {
					...
					case PLAYER_VISUAL_UPDATE: {
						...
						if(player != null) {
							...
							if(server.validatePlayerVisuals(player, spriteIndex, imageIndex, x, y) && player.room != -1) {
								...
								for(GamePlayer other : server.getPlayersInRoom(player.room)) {
									if(other == player) {
										continue;
									}
									server.sendPacket(other.connAddress, other.connPort, builder);
								}
							}
						}
					} break;
					...
					LOG.logger.severe("An internal error occured while processing a packet from " + packet.getAddress() + ":" + packet.getPort());
					...
					LOG.logger.severe("Bytes: " + Util.stringify(packet.getData(), packet.getLength()));
					LOG.logException(e);
				}
			}
		});
