...
public class OldGameServer
{
     ...
     */
    public void kickPlayer(UUID uuid, GamePlayer player, DatagramPacket inPacket, byte[] send, String message, boolean removeSession)
    {
        if (player.room != -1)
     ...
     */
    public boolean validatePlayerVisuals(UUID uuid, GamePlayer player, DatagramPacket inPacket, byte[] send, long now, int spriteIndex, int imageIndex, float x, float y)
    {
        ...
        while (running)
        {
            ...
            if(isRatelimited(packet.getAddress(), now)) {
            	if(ratelimited.add(packet.getAddress())) {
            		LOG.logger.warning("Client at " + packet.getAddress() + " is hitting ratelimits");
            		
            		for(GamePlayer player : sessions.values()) {
            			if(player.connAddress.equals(packet.getAddress())) {
            				sendPacket(player.connAddress, player.connPort, new PacketBuilder(OutboundPacketType.RATELIMIT_WARNING, send));
            			}
            		}
            	}
            	continue;
            ...
            if(ratelimited.remove(packet.getAddress())) {
            	LOG.logger.info("Client at " + packet.getAddress() + " is no longer hitting ratelimits");
            }
            ...
            try {
                ...
                case LOGIN:
                {
                    ...
                    // Add session
                    p = new GamePlayer(packet.getAddress(), packet.getPort(), uuid, id, now);
                    sessions.put(uuid, p);
                    if (properties.disallowSameIP)
                    ...
                    LOG.logger.info("Login packet from " + packet.getAddress().toString() + ", assigned ID " + id + " (" + uuid + ")");
                    break;
                }
            	...
            	LOG.logger.warning("Invalid message received from " + packet.getAddress() + ":" + packet.getPort());
            	if(uuid != null && p != null) {
